...
ResourceHandler::ResourceHandler(const std::string& docroot,
                                 const std::string& resroot)
    : _docroot(docroot), _resroot(resroot) {}

void ResourceHandler::handleRequest(Poco::Net::HTTPServerRequest& request,
...
void ResourceHandler::handleResource(Poco::Net::HTTPServerRequest& request,
                                     Poco::Net::HTTPServerResponse& response) {
  Poco::URI uri(request.getURI());
  Poco::Path path(_docroot, uri.getPath());
  Poco::File file(path);
...
void ResourceHandler::handleInternalResource(
